#!/bin/bash 
# new_event_debtor_comment.sh,
#
# ************************************************************
# * Script for inserting events and comments on debtors.
# * Please bear in mind to clean up temporary tables, and
# * archive (./adhoc/infile_backup)/ log files (./adhoc/sql_ldr_log). 
# * If you face problems please make sure sql loader works fine(using logs ./adhoc/sql_ldr_log)
# * If there are some procesures problems on PL/SQL please take a look on PINK.generic_trace_log table in database
# ************************************************************
#	
# DATE     WHO  		COMMENT
# -------- ----------	----------------------------------------------------------------------------------------------
# 20140317 PazikKamil 	Creation of script + Alexeys Kondratenko adjustments on code & directories
# ==========================================================================================================================================================
. $APP_SCRIPT/$APP_LOWER"_functions.sh"

infile_dir="${JOB_TMP}/adhoc" 
backup_infile_dir="${JOB_TMP}/adhoc/infile_backup" 
sqlldr_ctl_dir="${JOB_TMP}/adhoc/sql_ldr_ctl" 
sqlldr_log_dir="${JOB_TMP}/adhoc/sql_ldr_log" 
serv_req_log_dir="${JOB_TMP}/adhoc/serv_req_log" 
...
if ! [[ "${APP_COUNTRY}" = 'DK' || "${APP_COUNTRY}" = 'NO' ]] || ! [[ "${APP}" = 'pink' ]]; then
	...
	echo "Currently you are using $(WriteColor bold "${APP_COUNTRY}\\${APP}")"
	exit 1	
fi
